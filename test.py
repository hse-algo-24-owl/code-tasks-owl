...
class TestMatrixGenerator(unittest.TestCase):
    ...
    def test_zero(self):
        ...
        self.assertRaises(
            Exception, "Порядок матрицы меньше 1", get_random_matrix_and_det, 0, 1
        )
    ...
    def test_neg(self):
        ...
        self.assertRaises(
            Exception, "Порядок матрицы меньше 1", get_random_matrix_and_det, -1, 1
        )
    ...
    def test_det_with_numpy(self):
        ...
        for order in range(1, 10):
            for det in range(-70, 70):
                if det == 0:
                    continue
                gen_result = get_random_matrix_and_det(order, det)
